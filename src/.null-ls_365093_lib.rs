...
impl Expression {
    fn fetch_statement<T: Iterator<Item = Token>>(
        tokens: &mut T,
    ) -> Result<Expression, BadExpression> {
        match tokens.next().ok_or(BadExpression::Empty)? {
            ...
            Token::LoopStart => Expression::fetch_loop(tokens),
            Token::LoopEnd => Err(BadExpression::LoopNotOpened),
            Token::Comment(_) => Err(BadExpression::Comment),
        }
    }

    fn fetch_loop<T: Iterator<Item = Token>>(tokens: &mut T) -> Result<Expression, BadExpression> {
        let mut expressions = Vec::new();
        loop {
            match tokens.next().ok_or(BadExpression::Empty)? {
                Token::LoopEnd => return Ok(Expression::Loop(expressions)),
                _ => expressions.push(Expression::fetch_statement(tokens)?),
            }
        }
    }

    pub fn parse(tokens: Vec<Token>) -> Self {
        FromIterator::from_iter(tokens)
    }

    pub fn visit(&self, action: F)
        where
        V: Visitor,
        F: Fn(V) -> V
}
...
impl FromIterator<Token> for Expression {
    fn from_iter<T: IntoIterator<Item = Token>>(iter: T) -> Self {
        ...
        loop {
            match Expression::fetch_statement(&mut tokens) {
                Ok(expr) => expressions.push(expr),
                Err(BadExpression::LoopNotOpened) => {
                    expressions.clear();
                    break;
                }
                Err(BadExpression::Empty) => break,
                _ => (),
            }
        }
...
impl MemoryContext {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        ...
        let pointer = capacity / 2;

        MemoryContext { memory, pointer }
    }
}
